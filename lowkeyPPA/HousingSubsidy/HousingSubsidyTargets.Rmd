...
churnBounce <- read.csv("https://github.com/urbanSpatial/Public-Policy-Analytics-Landing/raw/master/DATA/Chapter6/churnBounce.csv")
...
```{r exploratory_continuous}
  housingsubsidy %>%
    dplyr::select(y_numeric,eunploy_rate, Time_On_FB, Daily_Internet_Usage, Age) %>%
    gather(Variable, value, -y_numeric) %>%
      ggplot(aes(y_numeric, value, fill=y_numeric)) + 
