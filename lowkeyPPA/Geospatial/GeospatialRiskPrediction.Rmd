...
vars_net <- 
  rbind(registeredMurals, parks, abandoned_buildings) %>%
...
```{r map of errors by random k fold and spatial cross validtion}
...
> Map isnt needed but figure out the kable table

```{r}

  ggplot() + geom_sf(data = tracts18, aes(fill = raceContext)) +
    scale_fill_manual(values = c("purple", "yellow"), name="Race Context") +
    labs(title = "Race Context") +
    mapTheme() + theme(legend.position="bottom")

st_join(tracts18, reg.ss.spatialCV) %>% 
  filter(!is.na(countNarco)) %>%
  group_by(raceContext, countNarco) %>%
  summarize(mean.MAPE = scales::percent(mean(countNarco.AbsError, na.rm = T))) %>%
  st_drop_geometry() %>%
  spread(raceContext, mean(countNarco.AbsError)) %>%
  kbl() %>%
  kable_classic()
