...
st_join(tracts17, reg.ss.spatialCV) %>%
 # filter(!is.na(countNarco)) %>%
  group_by(raceContext) %>%
  summarize(mean.MAE = mean(countNarco.AbsError)) %>%
  st_drop_geometry() %>%
  spread(, mean.MAE) %>%
  kable(caption = "Mean Error By Neighborhood") %>%
    kable_styling(bootstrap_options = c("striped", "hover", "condensed")) %>%
    row_spec(2, color = "black", background = "#FFFFFF") %>%
    row_spec(4, color = "black", background = "#FFFFFF")
